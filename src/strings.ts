...
const stringsEN = {
    ...
    fileCopy: {
        ...
        never: 'never',
        openBadge: 'open',
        moreHint: 'More – changes only in the browser',
        ...
        adviceMacDock: 'Add Strom to the Dock (in Safari: File → Add to Dock). The app there keeps its data more reliably than a Safari tab, which clears sites you have not opened for 7 days. It has storage of its own, so export the tree here and import it there. Keep saving to a file all the same.',
        adviceIosSafari: 'On iPhone and iPad, Safari clears the data of a site you have not opened for 7 days. Add Strom to the Home Screen (Share → Add to Home Screen): the app there is exempt. It has storage of its own, so export the tree here and import it there. Keep saving to a file (for example to iCloud Drive) all the same.',
        ...
        notice: (name: string) => `Changes to “${name}” are only in this browser, which may clear them. Save the tree to a file.`,
        noticeInstall: (name: string) => `Changes to “${name}” are only in this browser, which may clear them. Save the tree to a file — and install Strom as an app, so the browser keeps its data.`,
        noticeInstallMenu: (name: string) => `Changes to “${name}” are only in this browser tab, which may clear them. Save the tree to a file — and install Strom as an app (install icon in the address bar, or the browser menu), so the browser keeps its data.`,
        noticeFirefox: (name: string) => `Changes to “${name}” are only in this browser, which may clear them. Save the tree to a file — and allow Firefox to keep the data for good (site permissions in the address bar), or use Strom as an app in Chrome or Edge.`,
        noticeMacDock: (name: string) => `Changes to “${name}” are only in Safari, which clears sites unused for 7 days. Save the tree to a file — and add Strom to the Dock (File → Add to Dock).`,
        noticeIosSafari: (name: string) => `Changes to “${name}” are only in Safari, which clears sites unused for 7 days. Save the tree to a file, and add Strom to the Home Screen.`,
        save: 'Save to file',
        ...
        settingHint: 'When changes are only in browser storage that may be cleared. The toolbar icon stays either way.',
    },
    snapshots: {
        delete: 'Delete',
...
const stringsCZ: StringsType = {
    ...
    fileCopy: {
        ...
        never: 'nikdy',
        openBadge: 'otevřený',
        moreHint: 'Více – změny jen v prohlížeči',
        ...
        adviceMacDock: 'Přidejte si Strom do Docku (v Safari: Soubor → Přidat do Docku). Aplikace v Docku drží data spolehlivěji než karta Safari, které maže weby neotevřené 7 dní. Má ale vlastní úložiště — strom tady exportujte a v aplikaci naimportujte. Do souboru si ho i tak ukládejte.',
        adviceIosSafari: 'Na iPhonu a iPadu Safari maže data webu, který 7 dní neotevřete. Přidejte si Strom na plochu (Sdílet → Přidat na plochu): aplikace na ploše se to netýká. Má ale vlastní úložiště — strom tady exportujte a v aplikaci naimportujte. Do souboru si ho i tak ukládejte (třeba na iCloud Drive).',
        ...
        notice: (name: string) => `Změny ve stromu „${name}“ jsou jen v tomto prohlížeči, který je může smazat. Uložte si strom do souboru.`,
        noticeInstall: (name: string) => `Změny ve stromu „${name}“ jsou jen v tomto prohlížeči, který je může smazat. Uložte si strom do souboru — a nainstalujte si Strom jako aplikaci, aby prohlížeč data držel.`,
        noticeInstallMenu: (name: string) => `Změny ve stromu „${name}“ jsou jen v této kartě prohlížeče, který je může smazat. Uložte si strom do souboru — a nainstalujte si Strom jako aplikaci (ikona v adresním řádku nebo menu prohlížeče), aby prohlížeč data držel.`,
        noticeFirefox: (name: string) => `Změny ve stromu „${name}“ jsou jen v tomto prohlížeči, který je může smazat. Uložte si strom do souboru — a povolte Firefoxu trvalé úložiště (oprávnění webu v adresním řádku), nebo používejte Strom jako aplikaci v Chromu či Edgi.`,
        noticeMacDock: (name: string) => `Změny ve stromu „${name}“ jsou jen v Safari, které maže weby neotevřené 7 dní. Uložte si strom do souboru — a přidejte si Strom do Docku (Soubor → Přidat do Docku).`,
        noticeIosSafari: (name: string) => `Změny ve stromu „${name}“ jsou jen v Safari, které maže weby neotevřené 7 dní. Uložte si strom do souboru a přidejte si Strom na plochu.`,
        ...
        settingHint: 'Když jsou změny jen v úložišti prohlížeče, které se může smazat. Ikonka v liště zůstává tak jako tak.',
    },
    snapshots: {
        delete: 'Smazat',
...
const stringsDE: StringsType = {
    ...
    fileCopy: {
        ...
        never: 'nie',
        openBadge: 'geöffnet',
        moreHint: 'Mehr – Änderungen nur im Browser',
        ...
        adviceMacDock: 'Legen Sie Strom ins Dock (in Safari: Ablage → Zum Dock hinzufügen). Die App dort behält die Daten zuverlässiger als ein Safari-Tab, der Websites nach 7 Tagen ohne Besuch löscht. Sie hat aber einen eigenen Speicher — exportieren Sie den Stammbaum hier und importieren Sie ihn dort. Speichern Sie trotzdem in eine Datei.',
        adviceIosSafari: 'Auf iPhone und iPad löscht Safari die Daten einer Website, die Sie 7 Tage nicht geöffnet haben. Legen Sie Strom auf den Home-Bildschirm (Teilen → Zum Home-Bildschirm): Die App dort ist davon ausgenommen. Sie hat aber einen eigenen Speicher — exportieren Sie den Stammbaum hier und importieren Sie ihn dort. Speichern Sie trotzdem in eine Datei (etwa in iCloud Drive).',
        ...
        notice: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser, der sie löschen kann. Speichern Sie den Stammbaum in eine Datei.`,
        noticeInstall: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser, der sie löschen kann. Speichern Sie den Stammbaum in eine Datei — und installieren Sie Strom als App, damit der Browser die Daten behält.`,
        noticeInstallMenu: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser-Tab, der sie löschen kann. Speichern Sie den Stammbaum in eine Datei — und installieren Sie Strom als App (Symbol in der Adressleiste oder Browsermenü), damit der Browser die Daten behält.`,
        noticeFirefox: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser, der sie löschen kann. Speichern Sie den Stammbaum in eine Datei — und erlauben Sie Firefox den dauerhaften Speicher (Website-Berechtigungen in der Adressleiste), oder nutzen Sie Strom als App in Chrome oder Edge.`,
        noticeMacDock: (name: string) => `Änderungen an „${name}“ gibt es nur in Safari, das Websites nach 7 Tagen ohne Besuch löscht. Speichern Sie den Stammbaum in eine Datei — und legen Sie Strom ins Dock (Ablage → Zum Dock hinzufügen).`,
        noticeIosSafari: (name: string) => `Änderungen an „${name}“ gibt es nur in Safari, das Websites nach 7 Tagen ohne Besuch löscht. Speichern Sie den Stammbaum in eine Datei und legen Sie Strom auf den Home-Bildschirm.`,
        ...
        settingHint: 'Wenn Änderungen nur im Browserspeicher liegen, der gelöscht werden kann. Das Symbol in der Leiste bleibt so oder so.',
    },
    snapshots: {
        delete: 'Löschen',
