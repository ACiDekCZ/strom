...
const stringsEN = {
    ...
    // Versioned backups
    fileCopy: {
        indicatorTitle: 'Changes only in the browser, not in a file yet — the browser may clear them',
        indicatorShort: 'Only in browser',
        savedShort: 'Saved to file',
        persistentShort: 'Persistent storage',
        moreRowSub: 'Save a copy to a file',
        unsavedListTitle: 'Not saved to a file yet',
        never: 'never',
        openBadge: 'open',
        moreHint: 'More – changes are not saved',
        title: 'Where your data is',
        intro: 'Your trees are stored only in this browser, on this device — no account, no server. That also means the browser holds the only copy unless you save one to a file.',
        statePersistent: 'This browser keeps the storage for good: it will not clear the trees on its own.',
        stateNotPersistent: 'This storage is not persistent: the browser may clear it, for example when the device runs short of space.',
        stateUnsupported: 'This browser does not say whether it keeps the data for good — count on it being able to clear it.',
        copyNever: (name: string) => `“${name}” has not been saved to a file yet.`,
        copyAt: (name: string, when: string) => `“${name}” was last saved to a file (or opened from one) ${when}.`,
        copyChanged: 'Changes made since then are only in the browser.',
        copyUpToDate: 'Nothing has changed since then.',
        adviceInstall: 'Install Strom as an app: the browser then usually keeps its data for good. Save the tree to a file now and then all the same.',
        adviceInstallMenu: 'Install Strom as an app: in Chrome or Edge, use the install icon at the right end of the address bar, or the browser menu → Install Strom (Edge: Apps → Install this site as an app). An installed app usually gets storage the browser keeps for good. If Strom is installed already, open it from there rather than in a tab — the app window keeps the data. Save the tree to a file now and then all the same.',
        adviceFirefox: 'Firefox asks before it keeps a site’s data for good. Allow it: click the icon at the left end of the address bar → permissions → persistent storage. Firefox cannot install Strom as an app; for that, open Strom in Chrome or Edge and bring the tree over (export here, import there). Save the tree to a file now and then all the same.',
        adviceMacDock: 'Add Strom to the Dock (in Safari: File → Add to Dock). In a Safari tab, Safari may clear a site’s data when you do not use it for some time; the 7-day rule behind that does not apply to the app in the Dock. It has storage of its own, so export the tree here and import it there. Keep saving to a file all the same.',
        adviceIosSafari: 'On iPhone and iPad, Safari may clear a site’s data when you do not use it for some time (Apple’s 7-day rule, counted in a more complex way than simply days since the last visit). Add Strom to the Home Screen (Share → Add to Home Screen): the app there is normally not subject to this rule. It has storage of its own, so export the tree here and import it there. Keep saving to a file (for example to iCloud Drive) all the same.',
        adviceIosApp: 'iOS does not guarantee storage even to Home Screen apps — it may clear it when the device runs short of space. Save the tree to a file now and then, ideally to iCloud Drive.',
        adviceFile: 'Save the tree to a file now and then (Export), ideally to a cloud drive or another disk.',
        backupsNote: 'The backups in this app live in the same browser storage: they undo mistakes, but they go if the browser clears it. A file is the copy that lasts.',
        save: 'Save to file',
        quickHint: (name: string) => `The whole tree “${name}” with all its data and images goes into a JSON file you can open in Strom again at any time.`,
        quickHintAll: (n: number) => `All trees (${n}) with all their data and images go into one JSON file you can open in Strom again at any time.`,
        saveEncryptedBtn: 'Save encrypted',
        encryptLink: 'Encrypt with a password…',
        savedToast: (file: string) => `Saved to file ${file}`,
        appEncryptedHint: 'The data in the app is encrypted. A file without a password could be read by anyone who gets it.',
        saveAll: 'Save all trees',
        othersUnsaved: (names: string[]) => `These trees also have changes that are only in the browser: ${names.map(n => `“${n}”`).join(', ')}.`,
        install: 'Install app',
        installing: 'Strom is being installed as an app. Open it from there and import your tree if it is not there yet.',
        details: 'Where is my data?',
        menuUnsaved: 'Only in browser – not saved',
        notice: (name: string) => `Changes to “${name}” are only in this browser, which may clear them. Save the tree to a file now and then.`,
        noticeInstall: (name: string) => `Changes to “${name}” are only in this browser, which may clear them. Save the tree to a file now and then — and install Strom as an app, so the browser keeps its data.`,
        noticeInstallMenu: (name: string) => `Changes to “${name}” are only in this browser tab, which may clear them. Save the tree to a file now and then — and install Strom as an app (install icon in the address bar, or the browser menu), so the browser keeps its data.`,
        noticeFirefox: (name: string) => `Changes to “${name}” are only in this browser, which may clear them. Save the tree to a file now and then — and allow Firefox to keep the data for good (site permissions in the address bar), or use Strom as an app in Chrome or Edge.`,
        noticeMacDock: (name: string) => `Changes to “${name}” are only in Safari, which may clear a site’s data when you do not use it for some time. Save the tree to a file now and then — and add Strom to the Dock (File → Add to Dock).`,
        noticeIosSafari: (name: string) => `Changes to “${name}” are only in Safari, which may clear a site’s data when you do not use it for some time. Save the tree to a file now and then — and add Strom to the Home Screen.`,
        settingLabel: 'Remind me to save to a file',
        settingHint: 'A notice at the first change that is only in browser storage that may be cleared — again only after you save and change the tree again. The toolbar indicator stays either way.',
    },
    snapshots: {
        delete: 'Delete',
        ...
        restoredMissingImages: (n: number) => `Backup restored, but ${nEn(n, 'image was', 'images were')} no longer stored and ${n === 1 ? 'is' : 'are'} missing.`,
        tooBig: (tree: string, size: string) => `Backups of „${tree}“ do not fit the ${size} this device gives them (scans and photos) — only the newest three are kept. On this device it is better to turn its automatic backups off and export the tree to a file instead.`,
        tooBigAction: 'Backup settings',
    ...
    research: {
        ...
        waitingHint: 'Answer the agent where it runs — in its terminal or chat window.',
        since: (when: string) => `since ${when}`,
        justNow: 'just now',
        newChanges: (n: number) => nEn(n, 'new change', 'new changes'),
        readOnly: 'read-only',
        ...
        cardTitle: 'Ancestor research with an AI agent',
    ...
    encryption: {
        ...
        exportTitle: {
            json: 'Export – JSON',
            html: 'Export – standalone app (HTML)',
            gedcom: 'Export – GEDCOM',
            csv: 'Export – CSV',
            all: 'Export all',
        },
        exportLead: {
            json: 'Choose what goes into the file. Encrypting it with a password is optional.',
            html: 'The file opens in a browser, even offline. Encrypting it with a password is optional.',
            gedcom: 'The format for other genealogy programs. Choose what goes into the file.',
            csv: 'A table of people for Excel and similar programs. Choose what goes into the file.',
            all: 'All trees in one JSON file. Encrypting it with a password is optional.',
        },
        encryptToggle: 'Encrypt with a password',
        exportBtn: 'Export',
        exportEncryptedBtn: 'Export encrypted',
        contentCustom: 'Custom selection',
        contentEdit: 'Edit',
...
const stringsCZ: StringsType = {
    ...
    // Versioned backups
    fileCopy: {
        indicatorTitle: 'Změny jsou jen v prohlížeči, zatím ne v souboru — prohlížeč je může smazat',
        indicatorShort: 'Jen v prohlížeči',
        savedShort: 'Uloženo v souboru',
        persistentShort: 'Trvalé úložiště',
        moreRowSub: 'Uložte si kopii do souboru',
        unsavedListTitle: 'Zatím neuloženo v souboru',
        never: 'nikdy',
        openBadge: 'otevřený',
        moreHint: 'Více – změny nejsou uložené',
        title: 'Kde jsou vaše data',
        intro: 'Vaše stromy jsou uložené jen v tomto prohlížeči na tomto zařízení — bez účtu, bez serveru. Znamená to ale, že jedinou kopii má prohlížeč, dokud si ji neuložíte do souboru.',
        statePersistent: 'Tento prohlížeč úložiště drží trvale: stromy sám nesmaže.',
        stateNotPersistent: 'Toto úložiště není trvalé: prohlížeč ho může smazat, třeba když zařízení dochází místo.',
        stateUnsupported: 'Tento prohlížeč neříká, jestli data drží trvale — počítejte s tím, že je smazat může.',
        copyNever: (name: string) => `Strom „${name}“ zatím v žádném souboru uložený není.`,
        copyAt: (name: string, when: string) => `Strom „${name}“ jste naposledy uložili do souboru (nebo otevřeli ze souboru) ${when}.`,
        copyChanged: 'Změny od té doby jsou jen v prohlížeči.',
        copyUpToDate: 'Od té doby se nic nezměnilo.',
        adviceInstall: 'Nainstalujte si Strom jako aplikaci: prohlížeč pak data obvykle drží trvale. Strom si i tak občas uložte do souboru.',
        adviceInstallMenu: 'Nainstalujte si Strom jako aplikaci: v Chromu nebo Edgi ikonou instalace vpravo v adresním řádku, nebo v menu prohlížeče → Nainstalovat Strom (Edge: Aplikace → Nainstalovat tento web jako aplikaci). Nainstalovaná aplikace obvykle dostane úložiště, které prohlížeč drží trvale. Pokud už Strom nainstalovaný máte, otevírejte ho odtud, ne v kartě — data drží okno aplikace. Strom si i tak občas uložte do souboru.',
        adviceFirefox: 'Firefox se ptá, než data webu uloží trvale. Povolte to: ikona vlevo v adresním řádku → oprávnění → trvalé úložiště. Jako aplikaci Firefox Strom nainstalovat neumí; na to otevřete Strom v Chromu nebo Edgi a strom přeneste (tady exportujte, tam naimportujte). Strom si i tak občas uložte do souboru.',
        adviceMacDock: 'Přidejte si Strom do Docku (v Safari: Soubor → Přidat do Docku). V kartě Safari se data webu můžou smazat, když ho nějakou dobu nepoužíváte; na aplikaci v Docku se toto 7denní pravidlo nevztahuje. Má ale vlastní úložiště — strom tady exportujte a v aplikaci naimportujte. Do souboru si ho i tak ukládejte.',
        adviceIosSafari: 'Na iPhonu a iPadu může Safari smazat data webu, když ho nějakou dobu nepoužíváte (7denní pravidlo Applu, které se počítá složitěji než prostě dny od poslední návštěvy). Přidejte si Strom na plochu (Sdílet → Přidat na plochu): aplikace na ploše tomuto pravidlu standardně nepodléhá. Má ale vlastní úložiště — strom tady exportujte a v aplikaci naimportujte. Do souboru si ho i tak ukládejte (třeba na iCloud Drive).',
        adviceIosApp: 'iOS úložiště nezaručuje ani aplikacím na ploše — při nedostatku místa ho může vyklidit. Strom si občas uložte do souboru, nejlépe na iCloud Drive.',
        adviceFile: 'Strom si občas uložte do souboru (Exportovat), nejlépe do cloudu nebo na jiný disk.',
        backupsNote: 'Zálohy v aplikaci leží ve stejném úložišti prohlížeče: vrátí chybu, ale když prohlížeč úložiště smaže, zmizí s ním. Trvalá kopie je soubor.',
        save: 'Uložit do souboru',
        quickHint: (name: string) => `Celý strom „${name}“ se všemi údaji a obrázky se uloží do souboru JSON, který kdykoli znovu otevřete ve Stromu.`,
        quickHintAll: (n: number) => `Všechny stromy (${n}) se všemi údaji a obrázky se uloží do souboru JSON, který kdykoli znovu otevřete ve Stromu.`,
        saveEncryptedBtn: 'Uložit šifrovaně',
        encryptLink: 'Zašifrovat heslem…',
        savedToast: (file: string) => `Uloženo do souboru ${file}`,
        appEncryptedHint: 'Data v aplikaci máte zašifrovaná. Soubor bez hesla by si mohl přečíst kdokoli, kdo ho získá.',
        saveAll: 'Uložit všechny stromy',
        othersUnsaved: (names: string[]) => `Změny jen v prohlížeči mají i tyto stromy: ${names.map(n => `„${n}“`).join(', ')}.`,
        install: 'Nainstalovat',
        installing: 'Strom se instaluje jako aplikace. Otevřete ji a pokud v ní strom ještě není, naimportujte ho.',
        details: 'Kde jsou moje data?',
        menuUnsaved: 'Jen v prohlížeči – neuloženo',
        notice: (name: string) => `Změny ve stromu „${name}“ jsou jen v tomto prohlížeči, který je může smazat. Strom si průběžně ukládejte do souboru.`,
        noticeInstall: (name: string) => `Změny ve stromu „${name}“ jsou jen v tomto prohlížeči, který je může smazat. Strom si průběžně ukládejte do souboru — a nainstalujte si Strom jako aplikaci, aby prohlížeč data držel.`,
        noticeInstallMenu: (name: string) => `Změny ve stromu „${name}“ jsou jen v této kartě prohlížeče, který je může smazat. Strom si průběžně ukládejte do souboru — a nainstalujte si Strom jako aplikaci (ikona v adresním řádku nebo menu prohlížeče), aby prohlížeč data držel.`,
        noticeFirefox: (name: string) => `Změny ve stromu „${name}“ jsou jen v tomto prohlížeči, který je může smazat. Strom si průběžně ukládejte do souboru — a povolte Firefoxu trvalé úložiště (oprávnění webu v adresním řádku), nebo používejte Strom jako aplikaci v Chromu či Edgi.`,
        noticeMacDock: (name: string) => `Změny ve stromu „${name}“ jsou jen v Safari, které může smazat data webu, když ho nějakou dobu nepoužíváte. Strom si průběžně ukládejte do souboru — a přidejte si Strom do Docku (Soubor → Přidat do Docku).`,
        noticeIosSafari: (name: string) => `Změny ve stromu „${name}“ jsou jen v Safari, které může smazat data webu, když ho nějakou dobu nepoužíváte. Strom si průběžně ukládejte do souboru — a přidejte si Strom na plochu.`,
        settingLabel: 'Připomínat uložení do souboru',
        settingHint: 'Upozornění při první změně, která je jen v úložišti prohlížeče, jež se může smazat — znovu až po uložení a další změně. Indikátor v liště zůstává tak jako tak.',
    },
    snapshots: {
        delete: 'Smazat',
        ...
        restoredMissingImages: (n: number) => `Záloha obnovena. Chybějící obrázky (už nebyly uložené): ${n}.`,
        tooBig: (tree: string, size: string) => `Zálohy stromu „${tree}“ se nevejdou do ${size}, které jim toto zařízení dává (skeny a fotky) — drží se jen tři nejnovější. Na tomto zařízení je lepší automatické zálohy stromu vypnout a místo nich strom exportovat do souboru.`,
        tooBigAction: 'Nastavení záloh',
    ...
    events: {
        ...
        types: {
            ...
            firstCommunion: 'První svaté přijímání',
            barMitzvah: 'Bar micva',
            batMitzvah: 'Bat micva',
            ordination: 'Svěcení',
            adoption: 'Osvojení',
            naturalization: 'Naturalizace',
            will: 'Závěť',
            probate: 'Pozůstalostní řízení',
            title: 'Titul',
    ...
    research: {
        ...
        waitingHint: 'Odpovězte agentovi tam, kde běží — v jeho terminálu nebo okně chatu.',
        since: (when: string) => `od ${when}`,
        justNow: 'právě teď',
        newChanges: (n: number) => nCs(n, 'nová změna', 'nové změny', 'nových změn'),
        readOnly: 'jen pro čtení',
        showInTree: 'Ukázat ve stromu',
        ...
        cardTitle: 'Výzkum předků s AI agentem',
    ...
    personModal: {
        ...
        occupationField: 'Povolání / řemeslo',
        residenceField: 'Bydliště („z Lipan“)',
        originHint: 'Uloží se jako události povolání a bydliště; řemeslo je vidět na kartě v podrobné hustotě.',
    ...
    encryption: {
        ...
        exportTitle: {
            json: 'Export – JSON',
            html: 'Export – samostatná aplikace (HTML)',
            gedcom: 'Export – GEDCOM',
            csv: 'Export – CSV',
            all: 'Exportovat vše',
        },
        exportLead: {
            json: 'Vyberte, co se do souboru uloží. Zašifrování heslem je volitelné.',
            html: 'Soubor otevřete v prohlížeči i bez internetu. Zašifrování heslem je volitelné.',
            gedcom: 'Formát pro jiné genealogické programy. Vyberte, co se do souboru uloží.',
            csv: 'Tabulka osob pro Excel a podobné programy. Vyberte, co se do souboru uloží.',
            all: 'Všechny stromy do jednoho souboru JSON. Zašifrování heslem je volitelné.',
        },
        encryptToggle: 'Zašifrovat heslem',
        exportBtn: 'Exportovat',
        exportEncryptedBtn: 'Exportovat šifrovaně',
        contentCustom: 'Vlastní výběr',
        contentEdit: 'Upravit',
...
const stringsDE: StringsType = {
    ...
    // Versioned backups
    fileCopy: {
        indicatorTitle: 'Änderungen nur im Browser, noch nicht in einer Datei — der Browser kann sie löschen',
        indicatorShort: 'Nur im Browser',
        savedShort: 'In Datei gespeichert',
        persistentShort: 'Dauerhafter Speicher',
        moreRowSub: 'Kopie in einer Datei speichern',
        unsavedListTitle: 'Noch nicht in einer Datei gespeichert',
        never: 'nie',
        openBadge: 'geöffnet',
        moreHint: 'Mehr – Änderungen nicht gespeichert',
        title: 'Wo Ihre Daten liegen',
        intro: 'Ihre Stammbäume liegen nur in diesem Browser auf diesem Gerät — ohne Konto, ohne Server. Das heißt aber auch: Die einzige Kopie hat der Browser, solange Sie keine in einer Datei speichern.',
        statePersistent: 'Dieser Browser behält den Speicher dauerhaft: Er löscht die Stammbäume nicht von selbst.',
        stateNotPersistent: 'Dieser Speicher ist nicht dauerhaft: Der Browser kann ihn löschen, etwa wenn auf dem Gerät der Platz knapp wird.',
        stateUnsupported: 'Dieser Browser sagt nicht, ob er die Daten dauerhaft behält — rechnen Sie damit, dass er sie löschen kann.',
        copyNever: (name: string) => `„${name}“ ist noch in keiner Datei gespeichert.`,
        copyAt: (name: string, when: string) => `„${name}“ wurde zuletzt ${when} in eine Datei gespeichert (oder aus einer geöffnet).`,
        copyChanged: 'Änderungen seitdem gibt es nur im Browser.',
        copyUpToDate: 'Seitdem hat sich nichts geändert.',
        adviceInstall: 'Installieren Sie Strom als App: Der Browser behält die Daten dann meist dauerhaft. Speichern Sie den Stammbaum trotzdem ab und zu in eine Datei.',
        adviceInstallMenu: 'Installieren Sie Strom als App: in Chrome oder Edge über das Installationssymbol rechts in der Adressleiste oder im Browsermenü → Strom installieren (Edge: Apps → Diese Website als App installieren). Eine installierte App bekommt meist einen Speicher, den der Browser dauerhaft behält. Ist Strom schon installiert, öffnen Sie es von dort statt im Tab — das App-Fenster behält die Daten. Speichern Sie den Stammbaum trotzdem ab und zu in eine Datei.',
        adviceFirefox: 'Firefox fragt, bevor er die Daten einer Website dauerhaft speichert. Erlauben Sie es: Symbol links in der Adressleiste → Berechtigungen → dauerhafter Speicher. Als App installieren kann Firefox Strom nicht; dafür öffnen Sie Strom in Chrome oder Edge und übertragen den Stammbaum (hier exportieren, dort importieren). Speichern Sie den Stammbaum trotzdem ab und zu in eine Datei.',
        adviceMacDock: 'Legen Sie Strom ins Dock (in Safari: Ablage → Zum Dock hinzufügen). Im Safari-Tab kann Safari die Daten einer Website löschen, wenn Sie sie eine Zeit lang nicht nutzen; für die App im Dock gilt diese 7-Tage-Regel nicht. Sie hat aber einen eigenen Speicher — exportieren Sie den Stammbaum hier und importieren Sie ihn dort. Speichern Sie trotzdem in eine Datei.',
        adviceIosSafari: 'Auf iPhone und iPad kann Safari die Daten einer Website löschen, wenn Sie sie eine Zeit lang nicht nutzen (Apples 7-Tage-Regel, komplizierter gezählt als nur Tage seit dem letzten Besuch). Legen Sie Strom auf den Home-Bildschirm (Teilen → Zum Home-Bildschirm): Die App dort unterliegt dieser Regel normalerweise nicht. Sie hat aber einen eigenen Speicher — exportieren Sie den Stammbaum hier und importieren Sie ihn dort. Speichern Sie trotzdem in eine Datei (etwa in iCloud Drive).',
        adviceIosApp: 'iOS garantiert den Speicher auch Apps auf dem Home-Bildschirm nicht — bei Platzmangel kann es ihn leeren. Speichern Sie den Stammbaum ab und zu in eine Datei, am besten in iCloud Drive.',
        adviceFile: 'Speichern Sie den Stammbaum ab und zu in eine Datei (Exportieren), am besten in einen Cloud-Speicher oder auf ein anderes Laufwerk.',
        backupsNote: 'Die Sicherungen in der App liegen im selben Browserspeicher: Sie machen Fehler rückgängig, verschwinden aber, wenn der Browser ihn löscht. Die bleibende Kopie ist eine Datei.',
        save: 'In Datei speichern',
        quickHint: (name: string) => `Der ganze Stammbaum „${name}“ mit allen Daten und Bildern kommt in eine JSON-Datei, die Sie jederzeit wieder in Strom öffnen können.`,
        quickHintAll: (n: number) => `Alle Stammbäume (${n}) mit allen Daten und Bildern kommen in eine JSON-Datei, die Sie jederzeit wieder in Strom öffnen können.`,
        saveEncryptedBtn: 'Verschlüsselt speichern',
        encryptLink: 'Mit Passwort verschlüsseln…',
        savedToast: (file: string) => `In Datei ${file} gespeichert`,
        appEncryptedHint: 'Die Daten in der App sind verschlüsselt. Eine Datei ohne Passwort könnte jeder lesen, der sie bekommt.',
        saveAll: 'Alle Stammbäume speichern',
        othersUnsaved: (names: string[]) => `Auch diese Stammbäume haben Änderungen nur im Browser: ${names.map(n => `„${n}“`).join(', ')}.`,
        install: 'App installieren',
        installing: 'Strom wird als App installiert. Öffnen Sie sie und importieren Sie Ihren Stammbaum, falls er dort noch fehlt.',
        details: 'Wo sind meine Daten?',
        menuUnsaved: 'Nur im Browser – nicht gespeichert',
        notice: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser, der sie löschen kann. Speichern Sie den Stammbaum regelmäßig in eine Datei.`,
        noticeInstall: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser, der sie löschen kann. Speichern Sie den Stammbaum regelmäßig in eine Datei — und installieren Sie Strom als App, damit der Browser die Daten behält.`,
        noticeInstallMenu: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser-Tab, der sie löschen kann. Speichern Sie den Stammbaum regelmäßig in eine Datei — und installieren Sie Strom als App (Symbol in der Adressleiste oder Browsermenü), damit der Browser die Daten behält.`,
        noticeFirefox: (name: string) => `Änderungen an „${name}“ gibt es nur in diesem Browser, der sie löschen kann. Speichern Sie den Stammbaum regelmäßig in eine Datei — und erlauben Sie Firefox den dauerhaften Speicher (Website-Berechtigungen in der Adressleiste), oder nutzen Sie Strom als App in Chrome oder Edge.`,
        noticeMacDock: (name: string) => `Änderungen an „${name}“ gibt es nur in Safari, das die Daten einer Website löschen kann, wenn Sie sie eine Zeit lang nicht nutzen. Speichern Sie den Stammbaum regelmäßig in eine Datei — und legen Sie Strom ins Dock (Ablage → Zum Dock hinzufügen).`,
        noticeIosSafari: (name: string) => `Änderungen an „${name}“ gibt es nur in Safari, das die Daten einer Website löschen kann, wenn Sie sie eine Zeit lang nicht nutzen. Speichern Sie den Stammbaum regelmäßig in eine Datei — und legen Sie Strom auf den Home-Bildschirm.`,
        settingLabel: 'An das Speichern in eine Datei erinnern',
        settingHint: 'Ein Hinweis bei der ersten Änderung, die nur im löschbaren Browserspeicher liegt — erneut erst nach dem Speichern und einer weiteren Änderung. Die Anzeige in der Leiste bleibt so oder so.',
    },
    snapshots: {
        delete: 'Löschen',
        ...
        restoredMissingImages: (n: number) => `Sicherung wiederhergestellt, aber ${nDe(n, 'Bild war', 'Bilder waren')} nicht mehr gespeichert und ${n === 1 ? 'fehlt' : 'fehlen'}.`,
        tooBig: (tree: string, size: string) => `Die Sicherungen von „${tree}“ passen nicht in die ${size}, die dieses Gerät ihnen gibt (Scans und Fotos) — nur die drei neuesten bleiben. Auf diesem Gerät schalten Sie die automatischen Sicherungen besser aus und exportieren den Stammbaum stattdessen in eine Datei.`,
        tooBigAction: 'Sicherungseinstellungen',
    ...
    events: {
        ...
        types: {
            ...
            firstCommunion: 'Erstkommunion',
            barMitzvah: 'Bar Mitzwa',
            batMitzvah: 'Bat Mitzwa',
            ordination: 'Ordination',
            adoption: 'Adoption',
    ...
    research: {
        ...
        waitingHint: 'Antworten Sie dem Agenten dort, wo er läuft — in seinem Terminal oder Chatfenster.',
        since: (when: string) => `seit ${when}`,
        justNow: 'gerade eben',
        newChanges: (n: number) => nDe(n, 'neue Änderung', 'neue Änderungen'),
        readOnly: 'nur lesen',
        showInTree: 'Im Stammbaum zeigen',
        ...
        cardTitle: 'Ahnenforschung mit einem KI-Agenten',
    ...
    encryption: {
        ...
        exportTitle: {
            json: 'Export – JSON',
            html: 'Export – eigenständige App (HTML)',
            gedcom: 'Export – GEDCOM',
            csv: 'Export – CSV',
            all: 'Alles exportieren',
        },
        exportLead: {
            json: 'Wählen Sie, was in die Datei kommt. Das Verschlüsseln mit einem Passwort ist optional.',
            html: 'Die Datei öffnet sich im Browser, auch offline. Das Verschlüsseln mit einem Passwort ist optional.',
            gedcom: 'Das Format für andere Genealogieprogramme. Wählen Sie, was in die Datei kommt.',
            csv: 'Eine Personentabelle für Excel und ähnliche Programme. Wählen Sie, was in die Datei kommt.',
            all: 'Alle Stammbäume in einer JSON-Datei. Das Verschlüsseln mit einem Passwort ist optional.',
        },
        encryptToggle: 'Mit Passwort verschlüsseln',
        exportBtn: 'Exportieren',
        exportEncryptedBtn: 'Verschlüsselt exportieren',
        contentCustom: 'Eigene Auswahl',
        contentEdit: 'Bearbeiten',
