...
const stringsEN = {
    ...
    snapshots: {
        ...
        persistent: 'Storage is persistent — the browser will not clear these trees on its own.',
        notPersistent: 'The browser may clear this data when it runs short of space. Export the tree to a file regularly (Export → Save and back up). Installing Strom as an app, where the browser offers it, usually makes the storage persistent.',
...
const stringsCZ: StringsType = {
    ...
    snapshots: {
        ...
        persistent: 'Úložiště je trvalé — prohlížeč tyto stromy sám nesmaže.',
        notPersistent: 'Prohlížeč může tato data smazat, když mu dojde místo. Ukládejte strom pravidelně do souboru (Export → Uložit a zálohovat). Když Strom nainstalujete jako aplikaci (pokud to prohlížeč nabízí), úložiště obvykle trvalé bude.',
...
const stringsDE: StringsType = {
    ...
    snapshots: {
        ...
        persistent: 'Der Speicher ist dauerhaft — der Browser löscht diese Stammbäume nicht von selbst.',
        notPersistent: 'Der Browser kann diese Daten löschen, wenn ihm der Speicherplatz ausgeht. Exportieren Sie den Stammbaum regelmäßig in eine Datei (Export → Speichern und sichern). Wenn Sie Strom als App installieren (sofern der Browser das anbietet), wird der Speicher meist dauerhaft.',
